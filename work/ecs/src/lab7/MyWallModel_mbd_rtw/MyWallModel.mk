...
MODEL                = MyWallModel
MODULES              = ftm0_pwm_params.c ftm3_pwm_params.c mbd_main.c clock_S32K1xx.c ftm_chn_irq.c ftm_common.c ftm_fault_irq.c ftm_hw_access.c ftm_ic_driver.c ftm_mc_driver.c ftm_oc_driver.c ftm_pwm_driver.c ftm_qd_driver.c interrupt_manager.c lpit_driver.c pins_driver.c pins_port_hw_access.c power_manager.c power_manager_S32K1xx.c power_smc_hw_access.c s32k_clock_init.c startup.c system_S32K144.c
MAKEFILE             = MyWallModel.mk
MATLAB_ROOT          = C:\Program Files\MATLAB\R2020a_x64
ALT_MATLAB_ROOT      = C:\PROGRA~1\MATLAB\R2020A~1
START_DIR            = C:\Users\ecs\Desktop\embedded_systems_24\work\ecs\src\lab7
S_FUNCTIONS          = 
S_FUNCTIONS_LIB      = 
SOLVER               = 
NUMST                = 1
...
MEM_ALLOC            = RT_STATIC
COMPUTER             = PCWIN64
BUILDARGS            = COPY_MDLREF_INCLUDES=0 GENERATE_ERT_S_FUNCTION=0 INCLUDE_MDL_TERMINATE_FCN=1 COMBINE_OUTPUT_UPDATE_FCNS=1 MULTI_INSTANCE_CODE=0 INTEGER_CODE=0 ISPROTECTINGMODEL=NOTPROTECTING OPTS="-DTID01EQ=0"
MULTITASKING         = 0
MAT_FILE             = 0
GENERATE_ASAP2 	     = 0
EXT_MODE             = 0
...
FPU_USED                = 1
TARGET_LINKER           = S32K14x.ld
TARGET_MC_LIB           = S32K14x_AMMCLIB.a
TBX_SRCS                = 
TBX_VPATH               = 
TBX_INCLUDES            = 
...
ADD_INCLUDES = \
	...
	-$(include_opt)$(START_DIR)\MyWallModel_mbd_rtw \
...
ifeq ($(TARGET_COMPILER), GCC)
  ...
  ifeq ($(MAKEFILEBUILDER_TGT),1)
    ifneq ("$(wildcard ../instrumented/instrumentationInfo.mat)","")
      LD_FILE = ../instrumented/$(TARGET_LINKER)
      MC_LIB = ../instrumented/$(TARGET_MC_LIB)
    else
      LD_FILE = ../$(TARGET_LINKER)
      MC_LIB = ../$(TARGET_MC_LIB)
    endif
  else
    LD_FILE = $(TARGET_LINKER)
    MC_LIB = $(TARGET_MC_LIB)
...
CMSIS_INCLUDES  = \
    ...
    -$(include_opt)"../instrumented/src/sim/$(TARGET_FAMILY)" \
    -$(include_opt)"../instrumented/src/power/S32K1xx" \
    -$(include_opt)"../instrumented/$(DERIVATIVE)/include"
...
C_SRCS_LIST += $(TBX_SRCS)
C_SRCS = $(sort $(C_SRCS_LIST))

ifeq ($(MAKEFILEBUILDER_TGT),1)
  ifneq ("$(wildcard ../instrumented/instrumentationInfo.mat)","")
    S_SRCS = ../instrumented/startup_$(DERIVATIVE).s
  else
...
$(MODEL).elf: $(PREBUILT_OBJS) $(OBJS) $(MODELLIB) $(SHARED_LIB) $(LIBS) $(SRC_DEP) $(MODELREF_LINK_LIBS)
    ...
    ifeq ($(TARGET_COMPILER), IAR)
	$(LD) $(LD_OPTS) --config $(LD_FILE) $(IAR_TARGET_LINK_OPTS) $(PREBUILT_OBJS) $(OBJS) $(MODELLIB) $(MODELREF_LINK_LIBS) $(SHARED_LIB_LINK) $(LIBS) $(TOOL_LIB) $(TOOL_LIB) --map $(MODEL).map -o $@
    endif
    ifeq ($(TARGET_COMPILER), GreenHills Multi)
...
%.o : C:\Users\ecs\Documents\MATLAB\Add-Ons\Toolboxes\NXP_MBDToolbox_S32K1xx\code\mbdtbx_s32k\blocks\ftm/%.c
	@echo ### "C:\Users\ecs\Documents\MATLAB\Add-Ons\Toolboxes\NXP_MBDToolbox_S32K1xx\code\mbdtbx_s32k\blocks\ftm\$*.c"o
    ifeq ($(TARGET_COMPILER), GreenHills Multi)
